...
import { Truck, Eye, EyeOff, Shield, AlertTriangle } from 'lucide-react'
...
export function LoginPage() {
  ...
  function fillDemo(role: 'admin' | 'operator' | 'driver') {
    const emails = {
      admin: 'admin@shoppingacademias.com.br',
      operator: 'maria@shoppingacademias.com.br',
      driver: 'joao@shoppingacademias.com.br',
    }
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
      ...
      <div className="relative hidden lg:flex lg:w-1/2 flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 p-12">
        ...
        {/* Floating trucks decorative */}
        <div className="absolute left-8 top-16 opacity-10">
          <Truck className="h-24 w-24 text-blue-400 rotate-12" />
        </div>
        <div className="absolute bottom-16 right-8 opacity-10">
          <Truck className="h-32 w-32 text-blue-300 -rotate-6" />
        </div>
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="mt-3 text-lg text-slate-400">Controle de Frota</p>
          ...
          <div className="mt-12 grid grid-cols-2 gap-4 text-left">
            {[
              ...
              { icon: '⚡', title: 'Agilidade', desc: 'Checklist rápido no celular' },
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-sm">
            <div className="mb-6">
              ...
              <p className="mt-1 text-sm text-slate-400">
                Acesse o sistema de controle de frota
              </p>
            </div>
            ...
            {IS_DEMO_MODE && (
              <div className="mb-5 rounded-xl border border-blue-500/30 bg-blue-500/10 p-4">
                ...
                <p className="mt-1.5 text-xs text-blue-300">
                  O sistema está rodando sem Supabase real. Use os acessos abaixo:
                </p>
                <div className="mt-3 grid grid-cols-3 gap-2">
                  {['admin', 'operator', 'driver'].map((r) => (
                    <button
                      key={r}
                      type="button"
                      onClick={() => fillDemo(r as 'admin' | 'operator' | 'driver')}
                      className="rounded-lg border border-blue-400/30 bg-blue-600/20 px-2 py-1.5 text-xs font-medium text-blue-300 transition-colors hover:bg-blue-600/30"
                    >
                      {r === 'admin' ? '👑 Admin' : r === 'operator' ? '👤 Operador' : '🚛 Motorista'}
                    </button>
                  ))}
                </div>
              </div>
