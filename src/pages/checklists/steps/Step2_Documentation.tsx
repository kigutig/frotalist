...
const ITEMS = DEPARTURE_CHECKLIST_ITEMS.filter((i) => i.category === 'documentation')

export function Step2_Documentation({ form, onUpdateItem, onUpdateObservation }: StepProps) {
  const answered = ITEMS.filter((i) => form.items[i.key] && form.items[i.key] !== 'pending').length

  return (
    <div>
      <StepHeader
        title="Etapa 2 — Documentação"
        description="Verifique todos os documentos necessários"
        icon={FileText}
        count={ITEMS.length}
        answered={answered}
      />
